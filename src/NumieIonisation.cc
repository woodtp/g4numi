...
// ********************************************************************
//
// $Id: NumieIonisation.cc,v 1.1.2.1 2010/08/19 19:50:54 minervacvs Exp $
// GEANT4 tag $Name:  $
//
// -------------------------------------------------------------------
//
...
NumieIonisation::NumieIonisation(const G4String& name, G4ProcessType aType)
...
NumieIonisation::~NumieIonisation()
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void NumieIonisation::PreparePhysicsTable(const G4ParticleDefinition& part)
{
   NumieIonisation::InitialiseEnergyLossProcess(&part, theBaseParticle);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void NumieIonisation::InitialiseEnergyLossProcess(const G4ParticleDefinition* part,
                                                 const G4ParticleDefinition*)
{
   if(1 < verboseLevel) 
      G4cout << "NumieIonisation::InitialiseEnergyLossProcess ...." << G4endl;
   ...
   if(!isInitialised)
   {
      ...
      EmModel = em;
      NumieIonisation::AddEmModel(1, em, flucModel);
   ...
   //4double initialMass   = theParticle->GetPDGMass();
  ...
  G4cout << "  Building Physics Tables ... " << G4endl;
   ...
   theDEDXTable = NumieIonisation::BuildDEDXTable();
  ...
  if(1 < verboseLevel)
     G4cout << "NumieIonisation::InitialiseEnergyLossProcess - done" << G4endl;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4PhysicsTable* NumieIonisation::BuildDEDXTable()
{
  if(1 < verboseLevel) {
    G4cout << "NumieIonisation::BuildDEDXTable() for " << NumieIonisation::GetProcessName()
  ...
  for(size_t i=0; i<numOfCouples; i++)
  {
    ...
    if(1 < verboseLevel) 
      G4cout << "NumieIonisation::BuildDEDXVector flag=  " 
  ...
  if(1 < verboseLevel) {
    G4cout << "NumieIonisation::BuildDEDXTable(): table is built for "
           << theParticle->GetParticleName()
           << " and process " << NumieIonisation::GetProcessName()
           << G4endl;
...
void NumieIonisation::PrintInfo()
{
  if(EmModel)
...
G4VParticleChange* NumieIonisation::PostStepDoIt(const G4Track& track,
                                                  const G4Step&)
{
...
G4VParticleChange* NumieIonisation::AlongStepDoIt(const G4Track& track,
                                                   const G4Step& step)
{

   if(1 < verboseLevel) 
      G4cout << "NumieIonisation::AlongStepDoIt - called" << G4endl;

   G4double linLossLimit = 0.05;
   
  fParticleChange -> InitializeForAlongStep(track);

  NumieIonisation::InitialiseStep(track);
  ...
  G4cout << "Pre step point = " << step.GetPreStepPoint()->GetPhysicalVolume()->GetName()
         << " cell = " << step.GetPreStepPoint()->GetPhysicalVolume()->GetCopyNo() << G4endl;

  G4double range = NumieIonisation::GetScaledRangeForScaledEnergy(preStepScaledEnergy)*reduceFactor;
  ...
  if (length >= range)
  {
     //G4cout << "NumieIonisation::AlongStepDoIt - PROBLEM: Stopping particle: range = " << fRange << G4endl;
     return fParticleChange;
  }
  ...
  if(3 < verboseLevel)
  {
     ...
     G4cout << "AlongStepDoIt for "
            << NumieIonisation::GetProcessName() << " and particle "
            << d->GetParticleName()
  ...
  // Short step
  eloss = NumieIonisation::GetDEDXForScaledEnergy(preStepScaledEnergy)*length;
  ...
  if(eloss > preStepKinEnergy*linLossLimit)
  {
     ...
     eloss = (NumieIonisation::ScaledKinEnergyForLoss(r) - NumieIonisation::ScaledKinEnergyForLoss(x))/massRatio;
     ...
     if(3 < verboseLevel)
     {
        G4cout << "Long STEP: rPre(mm)= " << r/mm
               ...
               << " eloss(MeV)= " << eloss/MeV
               << " eloss0(MeV)= "
               << NumieIonisation::GetDEDXForScaledEnergy(preStepScaledEnergy)*length/MeV
               << G4endl;
     }
  }
  
  const G4DynamicParticle* dynParticle = track.GetDynamicParticle();
  G4VEmModel* currentModel = NumieIonisation::SelectModel(preStepScaledEnergy);

  if(!currentModel)
  {
     G4cout << "NumieIonisation::AlongStepDoIt - PROBLEM Can't get current model" << G4endl;
  }
  ...
  if((eloss + lowestKinEnergy) < preStepKinEnergy)
  {
     ...
     for(int i = 1; i < 100; ++i)
     {
        ...
        if(!flucMod)
        {
           G4cout << "NumieIonisation::AlongStepDoIt - PROBLEM Can't get current fluc model" << G4endl;
        }
        eflucloss = flucMod ->
     ...
     G4cout << "NumieIonisation::AlongStepDoIt - PROBLEM in trying to sample fluctuations" << G4endl;
  ...
  if(1 < verboseLevel) 
     G4cout << "NumieIonisation::AlongStepDoIt - done" << G4endl;
...
void NumieIonisation::SetStepFunction(G4double v1, G4double v2)
{
  dRoverRange = v1;
  finalRange = v2;
